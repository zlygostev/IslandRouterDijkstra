...
std::vector<uint8_t> generateElevationColormap()
{
    ...
    // Path value
    setColor(result, IPV_PATH, 255, 0, 0);

	// Water value
	setColor(result, IPV_WATER, 53, 160, 198);

	// Path value
	setColor(result, IPV_QUICK_PATH, 0, 200, 70);
	setColor(result, IPV_NORMAL_PATH, 180, 255, 0);
	setColor(result, IPV_SLOW_PATH, 255, 255, 0);
	setColor(result, IPV_TOO_SLOW_PATH, 255, 0, 0);

    
    
    // Sand to forest, levels 2 to 125
    blendColor(
        result,
		IPV_ELEVATION_BEGIN, 128,
        204, 201, 55,
        26, 130, 41);

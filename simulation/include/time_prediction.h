	...
	/*!
		...
		A few thinks are base for simulation of time - spent for one move.
			...
			3) How could measure the time t for path for a new cell: 
				...
				The gravitation that change speed of vehicle has a force  F = mg * sin(alpha).There is no any info about car mass.There is no any info about power of engine.There is no a lot of another thinks for good physical model.So let’s simplify – when angle vertical – the speed is 0 and car is not driving forward.When angle is 0 degree the speed is normal = 1 cell for straight way.It’s great fit for formula
					v = 1 * (1 – sin(alpha)).
...
private:
	...
	/*! Alpha is an angle of road line, that could be calculated as
		alpha = arctangent(delta(h) / delta(l)).
		There is no exact data about delta(h) and delta(l). We just know that delta(l) is a some 1 unit value
		and h is measured in some another units.
		...
		But, it also not a bad with delta(l) = delta(h). It something as speed 40 km/h and 8 units for island second. 
